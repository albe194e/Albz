...
import (
	...
	"github.com/albe194e/albz/app/core-go/crypt"
	domainidentity "github.com/albe194e/albz/app/core-go/domains/identity"
...
func (c *Controller) Register(ctx context.Context, name, username, password, profilePath string) error {
	...
	if _, err := c.IdentityService.GetLocalIdentity(ctx); err == nil {
	...
	err = c.IdentityService.UpsertLocalIdentity(ctx, domainidentity.UpsertLocalIdentityParams{
...
func (c *Controller) Login(ctx context.Context, username, password string) error {
	if c == nil || c.IdentityService == nil || c.State == nil || c.Relay == nil {
		return fmt.Errorf("controller is not fully initialized")
	}

	localIdentity, err := c.IdentityService.GetLocalIdentity(ctx)
	if err != nil {
		return err
	}

	if !localIdentityMatchesLogin(localIdentity, strings.TrimSpace(username)) {
		return fmt.Errorf("invalid username or password")
	}

	decryptedPrivateKey, err := crypt.DecryptDevicePrivateKey(crypt.EncryptedDeviceKeyRecord{
		EncryptedPrivateKey: localIdentity.EncryptedDevicePrivateKey,
		KDFSalt:             localIdentity.KdfSalt,
		KDFParams:           localIdentity.KdfParams,
	}, password)
	if err != nil {
		return fmt.Errorf("invalid username or password")
	}
	...
	if err := c.Relay.Connect(
		...
		nullStringValue(localIdentity.ContactCode),
		localIdentity.DevicePublicKey,
		decryptedPrivateKey,
	); err != nil {
	...
	_, err = c.CreateOrUpdateSession(ctx, localIdentity.UserID, localIdentity.DeviceID)
	if err != nil {
		if !previousServerConnected {
			_ = c.Relay.Close()
		}
...
func (c *Controller) CreateOrUpdateSession(ctx context.Context, userID string, deviceID string) (string, error) {
	...
	currentSession, err := c.IdentityService.GetCurrentSession(ctx)
	...
	now := time.Now()
	err = c.IdentityService.UpsertCurrentSession(ctx, domainidentity.UpsertCurrentSessionParams{
		SessionID: sessionID,
		UserID:    userID,
		DeviceID:  deviceID,
		CreatedAt: now.Unix(),
...
func (c *Controller) DeleteSession(ctx context.Context) error {
	return c.IdentityService.DeleteCurrentSession(ctx)
}

func (c *Controller) VerifySession(ctx context.Context) error {
	session, err := c.IdentityService.GetCurrentSession(ctx)
	if err != nil {
		return err
	}
	...
	if !localIdentityIsUsable(localIdentity) {
		return c.invalidateSession(ctx)
	}

	if session.UserID != localIdentity.UserID || session.DeviceID != localIdentity.DeviceID {
		return c.invalidateSession(ctx)
	}
...
func localIdentityMatchesLogin(identity domainidentity.LocalIdentity, login string) bool {
	if login == "" {
		return false
	}
...
func localIdentityIsUsable(identity domainidentity.LocalIdentity) bool {
